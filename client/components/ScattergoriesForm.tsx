...
export default function GameForm() {
  ...
  const [randomLetter] = useState(() => {
    const letters = ['P', 'S', 'N']
    return letters[Math.floor(Math.random() * 3)]
    //Reminder: this is taking the indexes of the letters array and creating a whole number by randomly 'mathing' a number between 0-1, times it by 3 and round down
  })
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    navigate('/scattergories/results', {
      state: { answer, letter: randomLetter },
    })
    ...
    //new learning: object shorthand - if your key and variable are the same, you can shortcut by writing once so {answer, letter: randomLetter} is the same as writing {answer: answer, ...}
  }
  ...
  return (
    <div>
      <h2 className="gameForm letter">Your chosen letter is {randomLetter}</h2>
      <div>
